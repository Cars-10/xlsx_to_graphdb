#!/usr/bin/env python3
"""
MCP Protocol compliant Windchill client for snowmobile data.
"""
...
class MCPWindchillClient:
    """MCP Protocol compliant Windchill client."""
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Available agents: {server_info.get('agents', [])}")
                logger.info(f"Available tools: {server_info.get('tools', 0)}")
                return True
            else:
    ...
    def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Call a specific MCP tool using proper protocol."""
        try:
            # MCP protocol format - direct tool call
            payload = {
                "tool": tool_name,
                "arguments": arguments
            }
            ...
            for endpoint in endpoints:
                try:
                    ...
                    logger.info(f"Tool {tool_name} via {endpoint}: {response.status_code}")
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(f"Tool call failed via {endpoint}: {response.status_code}")
                        
                except Exception as e:
                    logger.warning(f"Error calling {tool_name} via {endpoint}: {e}")
                    continue
            
            logger.error(f"All endpoints failed for tool {tool_name}")
            return None
    ...
    def list_parts(self, limit: int = 100) -> List[Dict[str, Any]]:
        """List all parts from Windchill."""
        result = self.call_tool("partmgmt_list_parts", {
            "limit": limit,
            "select": "Number,Name,State,Version,Iteration,Container"
        })
        
        if result:
            parts = result.get('results', [])
            logger.info(f"Found {len(parts)} total parts")
            return parts
        return []
...
def main():
    ...
    logger.info("Starting MCP Windchill client for snowmobile search")
    
    # Initialize client
    client = MCPWindchillClient()
    ...
    # First, let's get all parts to see what's available
    logger.info("Fetching all available parts...")
    all_parts = client.list_parts(limit=1000)
    
    if not all_parts:
        logger.warning("No parts found in Windchill system")
        return
    ...
    for term in search_terms:
        ...
        parts = client.search_parts(term, limit=100)
        snowmobile_parts.extend(parts)
        logger.info(f"Found {len(parts)} parts for '{term}'")
    
    # Also filter from all parts for any snowmobile-related terms
    snowmobile_keywords = ['snow', 'sno', 'mobile', 'track', 'ski', 'engine', 'chassis']
    
    for part in all_parts:
        part_name = part.get('Name', '').lower()
        part_number = part.get('Number', '').lower()
        
        if any(keyword in part_name or keyword in part_number for keyword in snowmobile_keywords):
            # Check if not already found
            part_num = part.get('Number', '')
            if not any(p.get('Number', '') == part_num for p in snowmobile_parts):
                snowmobile_parts.append(part)
    
    logger.info(f"Total snowmobile-related parts found: {len(snowmobile_parts)}")
    ...
    for i, part in enumerate(snowmobile_parts[:50]):  # Limit to first 50 for performance
        part_number = part.get('Number', part.get('number', ''))
        part_name = part.get('Name', part.get('name', ''))
        logger.info(f"Processing part {i+1}/{len(snowmobile_parts)}: {part_number} - {part_name}")
    ...
    for change in all_changes:
        ...
        if any(term in change_name or change_desc for term in ['snow', 'sno', 'mobile', 'track', 'ski', 'engine']):
    ...
    # Also create a summary for analysis
    summary = {
        'total_parts_found': len(snowmobile_parts),
        'parts_with_details': len(snowmobile_data['parts']),
        'total_changes': len(snowmobile_data['changes']),
        'total_change_objects': len(snowmobile_data['change_objects']),
        'bom_structures': len(snowmobile_data['bom_structures']),
        'sample_part_numbers': [p.get('Number', '') for p in snowmobile_parts[:10]]
    }
    
    with open('snowmobile_data_summary.json', 'w') as f:
        json.dump(summary, f, indent=2)
    
    logger.info("Summary saved to snowmobile_data_summary.json")

# Snowmobile → GraphDB Importer

## Why Use This Tool
- Load Windchill-style parts and BOM data from a single Excel workbook into a graph database quickly and repeatably.
...
## What You Can Do
- Parse `Snowmobile.xlsx` parts tabs and build a number→name index.
- Import parts into GraphDB with readable labels (`rdfs:label` and `urn:ontology:name`).
- Create parent→child relationships using:
  - Number-based BOM (`bom.csv`) for reliability, or
  - Name-based BOM (`bom_by_name.csv`) for human-friendly linkage.
- Generate name-based BOM from numbers and produce diagnostic reports:
  - `bom_by_name.csv` (generated from `bom.csv`)
  - `name_index.csv` (all known part numbers and names)
  - `bom_name_resolution_report.csv` (per-edge resolution status)
  - `skipped_names.log` (one line per unknown/ambiguous)
...
- Load by names (with auto-generation if needed):
  - `bash load_by_name.sh`
- Load by numbers (reliable, still shows names in the UI):
  - `python snowmobile_importer.py --excel Snowmobile.xlsx --bom bom.csv --url http://127.0.0.1:7200 --repo Snowmobile --quiet-missing-sheets`
- Enable debugging outputs:
  - `DEBUG_NAMES=1 bash load_by_name.sh`
...
- BOM by numbers: `bom.csv` with `Number` (parent) and `Component Id` (child).
- BOM by names: `bom_by_name.csv` with `Parent Name` and `Child Name`.
